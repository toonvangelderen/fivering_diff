...
int     Precision=40;              /* Number of sides per sphere, etc */
...
static float MaterialGreen[]   =   { 0.1, 0.6, 0.2, 0.7 };
...
// static float MaterialSome5[]    =   { 0.0, 0.3, 0.05, 1.0}; /*the dark green */
// static float MaterialSome5[]    =   { 0.0, 0.2, 0.30, 1.0}; /*the dark green/blue */
static float MaterialSome5[]    =   { 0.0, 0.25, 0.50, 1.0};
static float MaterialSome6[]    =   { 1.0, 0.67, 0.00, 0.8 }; /*old yellow*/
static float MaterialSome7[]    =   { 0.0, 0.67, 0.00, 0.8 }; /*unknown color*/
static float UvARed[]     =   { 0.73, 0.00, 0.19, 1.0 };
// static float MaterialSome6[]    =   { 1.0, 0.87, 0zz.20, 0.8 };/* banana yellow https://rgbcolorcode.com/color/FFDD33 */
...
void drawPatch(int ind,int isite, int ptype){
    ...
    glMaterialfv(GL_FRONT, GL_DIFFUSE,MaterialSome7);
...

{
   ...
   //glPushMatrix();
   //glTranslatef(pos[ind].r.x,pos[ind].r.y,pos[ind].r.z);
   //glScalef(pos[ind].radius+0.05,pos[ind].radius+0.05,pos[ind].radius+0.05);
   //switch (3) {
